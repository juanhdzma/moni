...
- `/api/admin/seed` and `/api/admin/truncate` reset all tables with fixture data — used for local testing, not production.
...
**Frontend (`js/`, `css/`, `index.html`)** — no modules, no bundler. Every file is loaded as a plain `<script>` tag in `index.html` in dependency order (config → state → services → features → ui-controls → app), and all functions/consts live in global scope. When adding a new file, add its `<script>` tag in the right position relative to what it depends on.
  ...
  - `js/features/*.js` — one file per domain tab (`dashboard`, `transacciones`, `deudas`, `inversiones`, `activos`, `recurrentes`, `admin`). Each owns its own render function(s) and modal form builder(s) that generate HTML via template literals and open through `openModal()`.
