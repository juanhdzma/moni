...
Frontend tests: `node --test tests/frontend/*.test.mjs` (no deps, no install — Node's built-in runner). They cover the money/date math only (`flujoCartera`, `flujoPatrimonio`, `netWorth`, `monthlyInvDelta`, `nextPaymentDate`, `toMensual`, `tieneSaldo`, `escHtml`, `fmtMoneyInput`); `tests/frontend/harness.mjs` loads the plain `public/js/` scripts into a `node:vm` context and pins `TZ=America/Bogota`, so timezone bugs surface on any machine. There is no linter, no type checker and no DOM/rendering test — verify UI changes by exercising the app.
...
- **"Composite actions"** (under the `── Composite actions` banner in main.py) are POST endpoints beyond plain CRUD that touch two tables in one SQLite transaction — e.g. `/api/deuda/{id}/pago` (loan payment), `/api/inv/{id}/aporte` (investment contribution), `/api/activo/{id}/venta` (asset sale). Each optionally inserts a linked row into `transacciones` (`registrar_tx` on the request body) so the ledger stays consistent with debt/investment/asset state. When adding a new money-moving action, follow this pattern rather than mutating state from the frontend.
  - Flags on the body carry *domain* meaning, and one flag can only mean one thing. `registrar_tx` is bookkeeping ("I already logged this elsewhere"); `capitaliza` (rendimiento) and `avanzar_cuota` (pago) are economics. Overloading `registrar_tx` to also decide where the money went would silently do the wrong thing for anyone who keeps part of the ledger outside Moni.
  - `/api/deuda/{id}/pago` rolls `proxima_cuota` forward a month when `avanzar_cuota` is set, and clears it when the balance reaches zero. Only the monthly cuota sets it — an extraordinary abono or a card payment must not move the due date. Without this the date froze and the debt eventually vanished from "próximas operaciones" for good.
...
- HTML is built with raw template literals + `escHtml()` for user content — no templating engine, no virtual DOM. `escHtml` escapes `< > & " '`, the single quote included, because generated `onclick="..."` attributes delimit their arguments with it.
...
- **Don't block pinch-zoom.** It has been re-broken twice by two different mechanisms: `user-scalable=no`/`maximum-scale` in the viewport meta plus `preventDefault` on gesture/touch events, and then `touch-action: pan-x pan-y` on `*` (that list without `pinch-zoom` disables the gesture just as hard). Fails WCAG 1.4.4, and the amounts are set in small mono type on mobile.
- **Anything clickable must be reachable by keyboard.** `<tr onclick>` and `<th onclick>` are not focusable on their own — they need `tabindex="0"`, a `role`, and an Enter/Space handler. Both regressed here before; the transaction rows and the sortable headers are the worked examples.
...
- **Never `new Date('YYYY-MM-DD')`.** That parses as midnight *UTC*, which in GMT-5 is 19:00 the day before — dates land on the wrong day and anything due today reads as overdue. Build from components (`new Date(y, m-1, d)`) or append `T12:00:00`; both are already used in the codebase. Same trap on the way out: `toISOString()` on a local date re-shifts it.
- **Never `setMonth(+n)` on a day-of-month that may not exist.** "31 de febrero" silently rolls into March and the series stays shifted from then on. Anchor the day and clamp it to the month's length — see `_sumarMeses()` (recurrentes.js) and `_sumar_un_mes()` (main.py).
