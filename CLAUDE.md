...
There is no linter or type checker, and no frontend tests — verify UI changes by exercising the app.
...
- **Money definitions live in one place**: `flujoCartera()` (what moves available cash — card `gasto`s don't, `transfer`s do) and `netWorth()` in dashboard.js. The evolution chart walks backwards from `netWorth()` subtracting `monthlyBalance()`, so if these drift apart the curve stops matching its own endpoint.
